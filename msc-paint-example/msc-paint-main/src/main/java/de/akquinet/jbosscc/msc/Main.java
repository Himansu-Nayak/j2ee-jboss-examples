...
public class Main {
    ...
    private void installPaintFrameService() throws ModuleLoadException {
        ...
        ServiceBuilder<PaintFrame> serviceBuilder = serviceSubTarget.addService(PaintFrameService.SERVICE_NAME,
                service);

        serviceBuilder.setInitialMode(Mode.ACTIVE);

        Set<ServiceName> dependencies = installShapeExtensions(serviceSubTarget.subTarget(),
                loadShapeExtensions(MAIN_MODULE));
        ...
        serviceBuilder.addListener(new AbstractServiceListener<PaintFrame>() {
            ...
            @Override
            public void transition(ServiceController<? extends PaintFrame> controller, Transition transition) {
                switch (transition) {
    ...
    private Set<ServiceName> installShapeExtensions(ServiceTarget serviceTarget,
        ...
        for (ShapeExtension<?> shapeExtension : extensions) {
            ServiceBuilder<?> serviceBuilder = shapeExtension.create(serviceTarget, PaintFrameService.SERVICE_NAME);
    ...
    private ServiceLoader<ShapeExtension> loadShapeExtensions(final String moduleName) throws ModuleLoadException {
        ModuleLoader moduleLoader = ModuleLoader.forClass(Main.class);

        Module loadModule = moduleLoader.loadModule(ModuleIdentifier.create(moduleName));
