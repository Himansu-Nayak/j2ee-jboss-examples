...
/**
 * A client to call the SingletonService via EJB remoting (AS7.1 / EAP6) to demonstrate the behaviour of asynchronous
 * invocations.
 * 
 * @author <a href="mailto:wfink@redhat.com">Wolf-Dieter Fink</a>
 */
public class StandaloneClient {
    private static final Logger LOGGER = Logger.getLogger(StandaloneClient.class.getName());
    /**
     ...
     */
    private StandaloneClient() throws NamingException {
        final Hashtable<String, String> jndiProperties = new Hashtable<>();
        jndiProperties.put(Context.URL_PKG_PREFIXES, "org.jboss.ejb.client.naming");
        final Context context = new InitialContext(jndiProperties);

        String lookupName = "ejb:/asynchronous-ejb/AsynchronousAccessBean!" + AsynchronousAccess.class.getName();
        LOGGER.info("Lookup Bean >" + lookupName);

        accessBean = (AsynchronousAccess) context.lookup(lookupName);
        lookupName = "ejb:/asynchronous-ejb/ParallelAccessBean!" + ParallelAccess.class.getName();
        LOGGER.info("Lookup Bean >" + lookupName);
    ...
    private void fireAndForget() throws InterruptedException {
        ...
        LOGGER.info(String.format(
                "The server log should contain a message at (about) %s, indicating that the call to the asynchronous bean completed.",
    ...
    /**
     * Demonstrate how to call an asynchronous EJB, and then perform another task whilst waiting for the result. If the
     * result is not present after the timeout of get(<timeout>) the result will be ignored.
     *
     * @throws TimeoutException
     *             Will be thrown if you change the timing
     */
    ...
    /**
     * Demonstrate how to call an asynchronous EJB and continue local work meanwhile. After finishing local work wait
     * for the result of the server call.<br/>
    ...
    /**
     * Invoke a remote synchronous EJB method. The remote method uses asynchronous calls internally, to parallelize it's
     * workload.
     */
    ...
    private void callAsyncWithFailure() throws InterruptedException {
        ...
        try {
            ...
            throw new RuntimeException("Unexpected failure during start asynchronous execution!", exception);
        }
        try {
            ...
            } else if (exception.getCause().getCause() instanceof IllegalAccessException) {
                // For releases < AS7.1.2 (EAP6.0.0) the Exception is covered by a second ExecutionException because of a bug
                LOGGER.info(
                        "Catch the covered Exception of the asynchronous execution, you may be using a release <= AS7.1.2 or EAP6.0.0!");
            } else {
                throw new RuntimeException("Unexpected ExecutionException during asynchronous call!", exception);
            }
        }
    }
