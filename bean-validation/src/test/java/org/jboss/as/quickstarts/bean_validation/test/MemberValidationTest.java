...
/**
 * Simple tests for Bean Validator. Arquillian deploys an WAR archive to the application server, which constructs
 * Validator object.
 * 
 * This object is injected into the tests so user can verify the validators are working. This example does not touch
 * validation on database layer, e.g. it is not validating uniqueness constraint for email field.
 * 
 * 
...
public class MemberValidationTest {
    ...
    public void testNameViolation() {
        ...
        Assert.assertEquals("Name was invalid", "must contain only letters and spaces",
    ...
    public void testEmailViolation() {
        ...
        Assert.assertEquals("One violation was found", 1, violations.size());
        Assert.assertEquals("Email was invalid", "not a well-formed email address",
                violations.iterator().next().getMessage());
    }
    ...
    public void testPhoneViolation() {
        ...
        Assert.assertEquals("One violation was found", 1, violations.size());
        Assert.assertEquals("Phone number was invalid", "size must be between 10 and 12",
                violations.iterator().next().getMessage());
    }
