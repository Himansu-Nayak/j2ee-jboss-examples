...
/**
 * Login module to make the decision if one user can ask for the current request to be switched to an alternative
 * specified user.
 *
...
public class DelegationLoginModule extends AbstractServerLoginModule {
    ...
    public void initialize(Subject subject, CallbackHandler callbackHandler, Map<String, ?> sharedState,
        ...
        try {
            ...
            throw new IllegalArgumentException(String.format("Unable to load properties '%s'", propertiesName),
                    exception);
        }
    }
    ...
    public boolean login() throws LoginException {
        ...
        if (credential instanceof OuterUserCredential) {
            ...
            log.info(">>>>>>>>>> delegationAcceptable: " + name);
     ...
     *
     * The following entries will be checked in the Properties in this order: - user@realm - This is an exact match for
     * the user / realm combination of the connection user. user@* - This entry allows a match by username for any
     * realm. *@realm - This entry allows for any user in the realm specified. * - This matches all users.
     *
     * Once an entry has been found the Properties will not be read again, even if the entry loaded does not allow
     * delegation.
     *
     * The value for the property is either '*' which means delegation to any user is allowed or a comma separate list
     * of users that can be delegated to.
     *
     * @param requestedUser
     *            - The user this request wants to be authorized as.
     * @param connectionUser
     *            - The use of the connection to the server.
    ...
    protected boolean delegationAcceptable(String requestedUser, OuterUserCredential connectionUser) {
        ...
        boolean permitted = allowedMappings.length == 1;
        permitted = permitted && "*".equals(allowedMappings[0]);
        if (permitted) {
