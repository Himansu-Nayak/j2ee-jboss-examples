...
public class XAService {
    ...
    /*
     * Ask the container to inject a persistence context corresponding to the database that will hold our key/value pair
     * table. The unit names corresponds to the one defined in the war archives' persistence.xml file
     */
    @PersistenceContext
    private EntityManager em;

    /*
     * Inject a UserTransaction for manual transaction demarcation (we could have used an EJB and asked the container to
     * manager transactions but we'll manage them ourselves so it's clear what's going on.
     */
    @Inject
        ...
        try {
            ...
            if (connection != null) {
                try {
                    ...
                    LOGGER.info("Error closing JMS connection: " + exception.getMessage());
                }
            }
        }
    }
     ...
     * @param delete
     *            if true then delete rows. If key is empty all rows are deleted.
     * @param key
     *            if not null then a pair is inserted into the database
     * @param value
    ...
    public String updateKeyValueDatabase(boolean delete, String key, String value) {
        ...
        try {
            ...
            result.append(exception.getCause() != null ? exception.getCause().getMessage() : exception.getMessage());
        } finally {
            try {
                ...
                result.append(" Transaction did not finish: ").append(exception.getMessage());
            }
        }
        return result.toString();
    }
