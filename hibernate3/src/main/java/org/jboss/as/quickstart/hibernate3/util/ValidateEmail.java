...
public class ValidateEmail implements Validator {
    ...
    @Override
    public void validate(FacesContext facesContext, UIComponent uiComponent, Object emailObject)
            throws ValidatorException {
        String emailAddress = (String) emailObject;
        /*
         * //Make sure it's not empty if (((String) emailAddress).isEmpty()) { FacesMessage facesMessage = new
         * FacesMessage(); facesMessage.setSummary("Email address is required."); facesMessage.setDetail(
         * "Email address is required."); facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
         * facesContext.addMessage("reg:email", facesMessage); throw new ValidatorException(facesMessage); }
         */
