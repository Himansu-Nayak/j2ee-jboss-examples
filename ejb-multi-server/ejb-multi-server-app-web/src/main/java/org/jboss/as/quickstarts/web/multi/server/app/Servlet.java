...
public class Servlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) {
        ...
        write(response,
                "It shows the name of the host instance (host-controller) and the unique server name on this host.<br/>");
        ...
        try {
            ...
            try {
                AppOne proxy = (AppOne) lookup(response, iCtx,
                        "ejb:jboss-ejb-multi-server-app-one/ejb//AppOneBean!com.org.jboss.app.one.AppOne");
                if (proxy != null) {
            ...
            try {
                AppTwo proxy = (AppTwo) lookup(response, iCtx,
                        "ejb:jboss-ejb-multi-server-app-two/ejb//AppTwoBean!com.org.jboss.app.two.AppTwo");
                if (proxy != null) {
                    write(response, "Invocation #1 return node.name => " + proxy.getJBossNodeName() + "<br/>");
                    // second invocation shows whether the same or a different node is reached
                    write(response, "Invocation #2 return node.name => " + proxy.getJBossNodeName() + "<br/>");
