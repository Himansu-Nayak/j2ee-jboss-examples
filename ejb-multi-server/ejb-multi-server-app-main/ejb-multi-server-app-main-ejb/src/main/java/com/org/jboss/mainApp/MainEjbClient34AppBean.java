...
import com.org.jboss.app.one.AppOne;
import com.org.jboss.app.two.AppTwo;
 ...
 *        The sub applications, deployed in different servers are called direct by using the ejb-client scoped context
 *        properties.
 *        </p>
 * 
...
public class MainEjbClient34AppBean implements MainApp {
    ...
    private void saveContextClose(Context iCtx) {
        if (iCtx != null) {
            try {
                ...
                LOGGER.error("InitialContext can not be closed", exception);
            }
        }
    }
    ...
    public String invokeAll(String text) {
        ...
        final StringBuilder result = new StringBuilder(
                "MainEjbClient34App[" + caller.getName() + "]@" + getJBossNodeName());
    ...
    /**
     * Invoke the AppOne with the scoped client context. The initial connection will use the 'quickuser1', to
     * differentiate the cluster connection it will be use the user 'quickuser2' to see if the clustered context is used
     * or not.
     *
     * @param text
    ...
    private String invokeAppOne(String text) {
        ...
        final Properties ejbClientContextProps = createAppOneICProperties("appMain->appOne_endpoint",
                "appOneConnection");
        ...
        try {
            ...
            final AppOne bean = (AppOne) iCtx
                    .lookup("ejb:ejb-multi-server-app-one/ejb//AppOneBean!" + AppOne.class.getName());
    ...
    /**
     * Invoke the App2 with different ejb-client context. The server AppTwoA will be called with the user quickuser1.
     * The server AppTwoB will be called with the user quickuser2. Both invocations are separate, there will no mix
     * between. Also the outbound-connection is not used.
     * 
     * @param text
     *            Simple text which will be logged at server side.
     * @return simple collection of the returned results
    ...
    private String invokeAppTwo(String text) {
        ...
        try {
            ...
            beanA = (AppTwo) iCtxA.lookup("ejb:ejb-multi-server-app-two/ejb//AppTwoBean!" + AppTwo.class.getName());
        } catch (NamingException exception) {
        ...
        try {
            ...
            beanB = (AppTwo) iCtxB.lookup("ejb:ejb-multi-server-app-two/ejb//AppTwoBean!" + AppTwo.class.getName());
        } catch (NamingException exception) {
    ...
    private Properties createAppOneICProperties(String endpoint, String connectionName) {
        final Properties ejbClientContextProps = new Properties();
        ejbClientContextProps.put("endpoint.name", endpoint);
