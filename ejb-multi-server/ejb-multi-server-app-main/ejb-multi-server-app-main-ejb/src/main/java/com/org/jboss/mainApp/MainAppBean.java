 ...
 * </p>
 * <p>
 * The sub applications, deployed in different servers are called direct or via indirect naming to hide the lookup name
 * and use a configured name via comp/env environment.
 * </p>
 * 
...
public class MainAppBean implements MainApp {
    ...
    @EJB(lookup = "ejb:ejb-multi-server-app-one/ejb//AppOneBean!com.org.jboss.app.one.AppOne")
    AppOne appOneProxy;

    @EJB(lookup = "ejb:ejb-multi-server-app-two/ejb//AppTwoBean!com.org.jboss.app.two.AppTwo")
    AppTwo appTwoProxy;
    ...
    public void init() {
        try {
            ...
            throw new RuntimeException("Could not initialize context", namingException);
        }
    }
    ...
    private String invokeAppOne(String text) {
        try {
            ...
            throw new RuntimeException("Could not invoke appOne", exception);
        }
    }
     ...
     * <ul>
     * <li>with the standard naming <i>ejb:jboss-ejb-multi-server-app-two/ejb//AppTwoBean!org.jboss.as.quickstarts
     * .ejb.multi.server.app.AppTwo</i></li>
     * <li><i>java:global/AliasAppTwo</i> the alias provided by the server configuration <b>this is not recommended</b>
     * </li>
    ...
    private String invokeAppTwo(String lookup, String text) throws NamingException {
        final AppTwo bean = (AppTwo) initialContext.lookup(lookup);
