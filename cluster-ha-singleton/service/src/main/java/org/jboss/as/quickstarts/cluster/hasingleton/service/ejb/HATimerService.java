...
/**
 * A service to start schedule-timer as HASingleton timer in a clustered environment. The service will ensure that the
 * timer is initialized only once in a cluster.
 *
...
public class HATimerService implements Service<String> {
    ...
    public void start(StartContext arg0) throws StartException {
        ...
        try {
            ...
            ((Scheduler) ic
                    .lookup("global/jboss-as-cluster-ha-singleton-service/SchedulerBean!org.jboss.as.quickstarts.cluster.hasingleton.service.ejb.Scheduler"))
                            .initialize("HASingleton timer @" + this.nodeName + " " + new Date());
        } catch (NamingException exception) {
            throw new StartException("Could not initialize timer", exception);
        }
    }
    ...
    public void stop(StopContext arg0) {
        if (!started.compareAndSet(true, false)) {
            ...
            try {
                ...
                LOGGER.error("Could not stop timer", exception);
            }
        }
    }
}
