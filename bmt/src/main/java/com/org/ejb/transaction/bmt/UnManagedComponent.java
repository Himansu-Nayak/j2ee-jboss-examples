...
public class UnManagedComponent {
    /*
     * Inject an entity manager factory. The reason we do not inject an entity manager (as we do in ManagedComponent) is
     * that the factory is thread safe whereas the entity manager is not.
     *
    ...
    public String updateKeyValueDatabase(String key, String value) {
        ...
        try {
             ...
             * to join the transaction. Compare this with ManagedComponent where the container automatically enlists the
             * EM with the transaction. The persistence context managed by the EM will then be scoped to the JTA
             * transaction which means that all entities will be detached when the transaction commits.
             */
            ...
            return t != null ? t.getMessage() : exception.getMessage();
        } catch (Exception exception) {
            /*
             * An application cannot handle any of the other exceptions raised by begin and commit so we just catch the
             * generic exception. The meaning of the other exceptions is:
             *
             * NotSupportedException - the thread is already associated with a transaction HeuristicRollbackException -
             * should not happen since the example is interacting with a single database HeuristicMixedException -
             * should not happen since the example is interacting with a single database SystemException - the TM raised
             * an unexpected error. There is no standard way of handling this error (another reason why CMT are
             * preferable to managing them ourselves)
             */
     ...
     *
     * @param entityManager
     *            an open JPA entity manager
     * @param key
     *            if null or zero length then list all pairs
     * @param value
     *            if key exists then associate value with it, otherwise create a new pair
