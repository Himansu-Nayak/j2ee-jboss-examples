...
@Stateless
public class CommentDaoBean extends AbstractDaoBean<Comment> implements CommentDao {
    ...
    public List<Comment> findComments(final BlogEntry blogEntry) {
        ...
        query.select(from).where(builder.equal(from.get(Comment_.blogEntry), blogEntry))
