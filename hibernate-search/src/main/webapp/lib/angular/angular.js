...
//////////////////////////////////////////
    var JQLitePrototype = JQLite.prototype = {
        ...
        toString: function () {
            var value = [];
            forEach(this, function (exception) {
            ...
            forEach(modulesToLoad, function (module) {
                ...
                if (isString(module)) {
                    ...
                    try {
                        ...
                        if (exception.message) exception.message += ' from ' + module;
                        throw exception;
                    }
                    ...
                    try {
                        ...
                        if (exception.message) exception.message += ' from ' + String(module[module.length - 1]);
                        throw exception;
                    }
                } else {
                    ...
                    for (var i = 0, ii = directives.length; i < ii; i++) {
                        ...
                        if (directive.templateUrl) {
                            ...
                            try {
                                ...
                                $exceptionHandler(exception, startingTag($compileNode));
                            }
                        }
                    ...
                    function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                        ...
                        for (i = 0, ii = preLinkFns.length; i < ii; i++) {
                            try {
                                ...
                                $exceptionHandler(exception, startingTag($element));
                            }
                        }
        ...
        function readNumber() {
            ...
            while (index < text.length) {
                ...
                if (ch == '.' || isNumber(ch)) {
                    ...
                    var peekCh = peek();
                    if (ch == 'exception' && isExpOperator(peekCh)) {
                        number += ch;
                    } else if (isExpOperator(ch) &&
                        peekCh && isNumber(peekCh) &&
                        number.charAt(number.length - 1) == 'exception') {
                        number += ch;
                    } else if (isExpOperator(ch) &&
                        (!peekCh || !isNumber(peekCh)) &&
                        number.charAt(number.length - 1) == 'exception') {
 ...
 *     });
    ...
    function formatNumber(number, pattern, groupSep, decimalSep, fractionSize) {
        ...
        if (numStr.indexOf('exception') !== -1) {
            var match = numStr.match(/([\d\.]+)exception(-?)(\d+)/);
