...
1. Define the security domain. This can be done either in the `security` subsystem of the `standalone.xml` configuration file or in the `WEB-INF/jboss-web.xml` configuration file. This quickstart uses the `other` security domain which is provided by default in the `standalone.xml` file:
...
Refer http://developers.redhat.com/quickstarts/eap/ejb-security/
...
The application will be running at the following URL <http://localhost:8080/ejb-security-declarative/>.
...
3. Now close and reopen the browser session and access the application using the `user1`/`password1!` credentials. In this case, the Servlet, which only allows the `guest` role, restricts the access and you get a security exception similar to the following: 
